...
class SdlTable extends PureComponent {
  ...
  render() {
    ...
    const { tableInfo, searchForm, keys, dispatch, configId, btnsAuthority } = this.props;
          ...
          <div>
            {
              this._SELF_.btnEl.map((item, index) => {
                if (item.type === "edit" && btnsAuthority["edit"]) {
                ...
                if (item.type === "view" && btnsAuthority["view"]) {
                // if (item.type === "view") {
                ...
                if (item.type === "del" && btnsAuthority["del"]) {
                  return (<Fragment key={item.type}>
                    <Tooltip title="删除">
                      <Popconfirm
                        placement="left"
