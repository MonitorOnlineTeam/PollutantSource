...
export default Model.extend({
    ...
    effects: {
        * queryLxSearchResult({
            ...
            if (searchName === payload.value) { return; }
        ...
        * querySearchInfo({
            ...
            result.data.map(info => {
                ...
                if (info.Type === 1) {
                    ...
                    if (info.OtherInfo.split('&#&')[16] !== '') { model.operationName = ('运维人名称：' + info.OtherInfo.split('&#&')[16]); }
                    if (info.OtherInfo.split('&#&')[17] !== '') { model.operationtel = ('运维人电话：' + info.OtherInfo.split('&#&')[17]); }
                    if (info.OtherInfo.split('&#&')[18] !== '') { model.operationtask = ('运维逾期任务：' + info.OtherInfo.split('&#&')[18]); }
                    if (info.OtherInfo.split('&#&')[19] !== '') { model.isStop = ('是否停产：' + info.OtherInfo.split('&#&')[19]); }
                    if (info.OtherInfo.split('&#&')[20] !== '') { model.isfault = ('是否故障：' + info.OtherInfo.split('&#&')[20]); }
                    if (info.OtherInfo.split('&#&')[21] !== '') { model.isoperation = ('是否运维中：' + info.OtherInfo.split('&#&')[21]); }
                } else if (info.Type === 6) {
                    model.code = info.OtherInfo.split('&#&')[5];
                    info.OtherInfo = info.OtherInfo.replace(reg, "<span style='color:red'>" + payload.value + '</span>');
                    model.type = '监测点';
                    ...
                    if (info.OtherInfo.split('&#&')[13] !== '') { model.operationName = ('运维人名称：' + info.OtherInfo.split('&#&')[13]); }
                    if (info.OtherInfo.split('&#&')[14] !== '') { model.operationtel = ('运维人电话：' + info.OtherInfo.split('&#&')[14]); }
                    if (info.OtherInfo.split('&#&')[15] !== '') { model.operationtask = ('运维逾期任务：' + info.OtherInfo.split('&#&')[15]); }
                    if (info.OtherInfo.split('&#&')[16] !== '') { model.isStop = ('是否停产：' + info.OtherInfo.split('&#&')[16]); }
                    if (info.OtherInfo.split('&#&')[17] !== '') { model.isfault = ('是否故障：' + info.OtherInfo.split('&#&')[17]); }
                    if (info.OtherInfo.split('&#&')[18] !== '') { model.isoperation = ('是否运维中：' + info.OtherInfo.split('&#&')[18]); }
                }
