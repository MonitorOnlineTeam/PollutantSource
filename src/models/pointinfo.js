...
export default Model.extend({
  ...
  state: {
    ...
    ChildCems: [], //CEMS监测子系统
    Analyzers: [], //监测仪器信息
    WaterFunctionAreaList: [], //碼表，功能区返回数据
    WaterFunctionAreaList_requstresult: [], //功能区返回 标识
  },
  subscriptions: {
    setup({
      dispatch,
  ...
  effects: {
    ...
    * getWaterFunctionAreaList({
      ...
      const result = yield call(getWaterFunctionAreaList, {});
      debugger

      if (result.requstresult === '1') {
        yield update({
          WaterFunctionAreaList_requstresult: result.requstresult,
          WaterFunctionAreaList: result.data,
        });
      } else {
        yield update({
          WaterFunctionAreaList_requstresult: result.requstresult,
          WaterFunctionAreaList: [],
        });
      }
      payload.callback && payload.callback();
    },
    ...
    * getmaininstrumentName({
      ...
      if (DataInfo !== null && DataInfo.requstresult == EnumRequstResult.Success) {
        if (DataInfo.data !== null) {
          yield update({
            MainInstrumentName: DataInfo.data
          });
          callback();
        }
      }
