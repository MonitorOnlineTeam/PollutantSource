...
export async function addpoint(params) {
  const body = {
    ...
    AirArea:params.AreaFunctionCode,
  };
...
// 获取功能区列表
export async function getWaterFunctionAreaList(params) {
  const body = {
  };
  const result = post('/api/rest/PollutantSourceApi/PPointAndData/getWaterFunctionAreaList', body, null);
  return result === null ? {
    data: null
  } : result;
}
